...
from tensorflow.keras.layers import (
    Dense, Flatten, Conv2D, MaxPool2D, BatchNormalization, ReLU
)

def make_convNet(input_shape, depth, n_classes=10, init_channels=64, layer_initializer=None):
    '''
    ...
    conv_shapes = [input_shape] + [
        [input_shape[1] // (2**(i)), input_shape[2]// (2**(i)), init_channels * (2**i)] for i in range(depth-2)
    ]
    ...
    for i in range(depth-1):
        ...
        conv_net.add(Conv2D(
            filters=n_filters, input_shape=conv_shapes[i], kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_initializer=layer_init
        ))
    ...
    conv_net.add(Dense(units=n_classes, activation='softmax', kernel_initializer=layer_init))
