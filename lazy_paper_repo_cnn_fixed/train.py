...
# change: changed num_workers=0 from 2
# Windows cannot run this DataLoader in 'num_workers' more than 0
...
if args.subset>0:
    ...
    trainset = torchvision.datasets.CIFAR10(root='./data', train=True, download=True, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.bs, num_workers=0,sampler=SubsetRandomSampler(range(args.subset)))



testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=args.bs_test, shuffle=False, num_workers=0)
...
# change: ':' in file name causes OSError
name_log_txt = name_log_txt.replace(':','-')
