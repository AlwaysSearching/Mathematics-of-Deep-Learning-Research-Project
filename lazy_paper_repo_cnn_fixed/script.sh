...
do
py train.py --scaling_factor $ALPHA  --lr $LR --gain 1.0 --schedule 'b' --loss 'mse' --length 100 --precision 'double'
done
done

# Obtain the SVD of the tangent kernel for cifar and random features

py extract_kernel.py --bs 9 --data 'random' --subset 495
py extract_kernel.py --bs 9 --subset 495
